...
import { Car, Key, SteeringWheel, GasPump, RoadHorizon, MapPin, InstagramLogo, FacebookLogo, ArrowRight } from '@phosphor-icons/react';
  ...
  return (
    <div className="min-h-screen font-sans selection:bg-blue-600 selection:text-white overflow-x-hidden bg-white text-slate-900">
      ...
      <nav className="w-full px-6 py-4 bg-white/80 backdrop-blur-md border-b border-slate-200 flex justify-between items-center sticky top-0 z-50">
        <div className="text-xl font-bold tracking-tight text-slate-900 font-geist flex items-center gap-3">
        ...
        <a 
          href={`https://wa.me/${config.contact.whatsapp}?text=Halo%20${encodeURIComponent(config.businessName)},%20saya%20ingin%20menyewa%20kendaraan`}
          target="_blank"
          rel="noreferrer"
        ...
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center relative z-10">
          ...
          <div className="lg:col-span-6 flex flex-col items-start">
            ...
            <motion.h1 
              ...
              className="text-5xl sm:text-6xl md:text-7xl lg:text-[5.5rem] font-bold tracking-tighter leading-[1.05] text-slate-900 mb-8 font-geist"
            >
              {config.tagline}
            </motion.h1>
            ...
            >
              ...
              <div className="w-full aspect-[4/3] md:aspect-video relative rounded-lg overflow-hidden bg-slate-100">
                {config.images?.gallery && config.images.gallery.length > 0 ? (
                  ...
                  <Image src={config.images.hero} alt="Fleet" fill className="object-cover" />
                ) : (
      ...
      <section className="py-24 px-6 max-w-[1600px] mx-auto relative z-10">
        <div className="mb-12 flex items-end justify-between border-b border-slate-200 pb-6">
          <div>
            <h2 className="text-3xl font-bold tracking-tight font-geist text-slate-900 mb-2">Fleet Options</h2>
          ...
          {config.services.map((svc, i) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-2xl font-bold tracking-tight mb-3 text-slate-900 font-geist group-hover:text-blue-600 transition-colors">{svc.name}</h3>
        ...
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h3 className="text-xl font-bold text-white font-geist tracking-tight">{config.businessName}</h3>
            </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <a href={config.contact.mapsLink} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-white transition-colors text-sm font-medium w-max">
              Open in Maps &rarr;
            </a>
          </div>
          ...
          <div className="flex flex-col gap-4">
             ...
             <div className="flex flex-col gap-3 font-medium text-sm">
               <a href={`https://wa.me/${config.contact.whatsapp}?text=Halo%20${encodeURIComponent(config.businessName)},%20saya%20ingin%20menyewa%20kendaraan`} target="_blank" rel="noreferrer" className="text-slate-300 hover:text-white transition-colors w-max">
                 WhatsApp Support
               </a>
               {config.contact.instagram && (
                 <a href={config.contact.instagram} target="_blank" rel="noreferrer" className="text-slate-300 hover:text-white transition-colors flex items-center gap-2 w-max">
                   <InstagramLogo weight="fill" size={18} /> Instagram
                 </a>
               )}
               {config.contact.facebook && (
                 <a href={config.contact.facebook} target="_blank" rel="noreferrer" className="text-slate-300 hover:text-white transition-colors flex items-center gap-2 w-max">
