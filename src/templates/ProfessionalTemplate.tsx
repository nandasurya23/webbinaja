    ...
    <div className="w-full min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-900 selection:text-white overflow-x-hidden">
      ...
      <nav className="w-full px-6 py-6 md:px-12 md:py-8 flex justify-between items-center bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xl md:text-2xl font-semibold tracking-tight text-slate-900 font-sans">
            {config.businessName}
          </span>
        </div>
        <a 
      ...
      <section className="px-6 py-24 max-w-[1600px] mx-auto">
        <div className="mb-12 md:mb-16 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-slate-900 font-sans mb-4">Layanan Kami</h2>
          ...
          {config.services.map((svc, i) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-2xl font-bold text-slate-900 mb-4 font-sans">{svc.name}</h3>
      ...
      {config.testimonials && config.testimonials.length > 0 && (
        <section className="px-6 py-24 bg-white border-y border-slate-200">
          <div className="max-w-[1600px] mx-auto">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight text-slate-900 font-sans mb-16 text-center">Apa Kata Pasien</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <footer className="px-6 py-24 bg-slate-50">
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 lg:gap-24">
          ...
          <div className="md:col-span-6 lg:col-span-4 flex flex-col gap-6">
            ...
            <a href={sanitizeUrl(config.contact.mapsLink)} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-blue-700 font-semibold hover:text-blue-900 transition-colors w-max mt-2">
          ...
          <div className="md:col-span-12 lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4 text-lg font-semibold text-slate-700">
              <a href={`https://wa.me/${sanitizeWhatsapp(config.contact.whatsapp)}?text=Halo%20${encodeURIComponent(config.businessName)},%20saya%20ingin%20buat%20janji%20temu/konsultasi`} target="_blank" rel="noreferrer" className="flex items-center gap-3 hover:text-blue-700 transition-colors">
                 WhatsApp
              </a>
              {config.contact.instagram && (
                <a href={sanitizeUrl(config.contact.instagram)} target="_blank" rel="noreferrer" className="flex items-center gap-3 hover:text-blue-700 transition-colors">
                  <InstagramLogo size={24} /> Instagram
                </a>
              )}
              {config.contact.facebook && (
                <a href={sanitizeUrl(config.contact.facebook)} target="_blank" rel="noreferrer" className="flex items-center gap-3 hover:text-blue-700 transition-colors">
