...
export const RestaurantTemplate = ({ config }: { config: CustomerConfig }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0c0d0a] text-[#f4f1eb] font-sans selection:bg-[#d4af37] selection:text-black overflow-x-hidden">
      
      {/* Editorial Navbar */}
      <nav className="fixed top-0 left-0 w-full px-8 py-6 md:px-16 flex justify-between items-center z-50 bg-[#0c0d0a]/80 backdrop-blur-md border-b border-[#d4af37]/20 transition-all">
        <div className="text-xl md:text-2xl font-light tracking-[0.2em] uppercase font-outfit text-white">
          {config.businessName}
        </div>
        
        <div className="hidden md:flex items-center gap-8 text-[10px] font-medium tracking-[0.3em] uppercase text-[#8c8a85]">
          <a href="#about" className="hover:text-[#d4af37] transition-colors">Story</a>
          <a href="#services" className="hover:text-[#d4af37] transition-colors">Menu</a>
          {config.catalog && <a href="#catalog" className="hover:text-[#d4af37] transition-colors">Specials</a>}
          {config.images?.gallery && <a href="#gallery" className="hover:text-[#d4af37] transition-colors">Ambiance</a>}
        </div>
      ...
      {/* Fine Dining Menu */}
      <section id="services" className="py-32 md:py-48 px-8 md:px-16 bg-[#11120e] relative border-y border-[#d4af37]/10">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-24 md:mb-32">
            <span className="text-[10px] font-outfit uppercase tracking-[0.4em] text-[#d4af37] mb-6">
              Menu Degustation
            </span>
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-baseline gap-4 md:gap-12 relative z-10">
                  ...
                  <div className="md:w-1/4 flex md:justify-end shrink-0">
                    <span className="text-xl md:text-2xl text-[#d4af37] font-light font-outfit tracking-widest">{svc.price}</span>
                  </div>
                </div>
                
                <div className="flex flex-col md:flex-row justify-between items-start gap-4 md:gap-12 mt-2">
                  <div className="md:w-3/4">
                    {svc.desc && <p className="text-[#8c8a85] text-base md:text-lg max-w-2xl leading-relaxed italic">{svc.desc}</p>}
                  </div>
      ...
      {config.images?.gallery && config.images.gallery.length > 0 && (
        <section id="gallery" className="py-32 md:py-48 px-8 md:px-16 max-w-7xl mx-auto flex flex-col md:flex-row gap-16 md:gap-24 items-center relative border-t border-[#d4af37]/10">
           ...
           <div className="w-full md:w-[55%] flex flex-col gap-16 md:gap-24 relative">
             <div className="absolute -left-12 top-1/2 w-24 h-[1px] bg-[#d4af37]/30 hidden md:block"></div>
      ...
      <footer className="pt-32 pb-16 px-8 md:px-16 max-w-7xl mx-auto flex flex-col justify-between items-center text-center gap-16 border-t border-[#d4af37]/20">
        
        <div className="flex flex-col items-center">
          <h3 className="text-2xl font-light tracking-[0.2em] uppercase mb-8 font-outfit text-white">{config.businessName}</h3>
          <p className="text-[#8c8a85] text-sm tracking-widest max-w-sm mb-12 leading-relaxed uppercase">
        ...
        <div className="flex flex-col gap-12 w-full">
          ...
          <div className="text-[#595854] text-[9px] uppercase tracking-[0.4em] pt-8 border-t border-[#d4af37]/10">
