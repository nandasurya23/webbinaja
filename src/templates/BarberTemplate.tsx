...
import { ArrowRight, ArrowDownRight, MapPin, InstagramLogo, FacebookLogo } from '@phosphor-icons/react';
  ...
  return (
    <div className="w-full min-h-screen bg-[#0a0a0a] text-zinc-100 font-sans selection:bg-white selection:text-black overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-[#0a0a0a]/80 backdrop-blur-md border-b-2 border-zinc-800 px-6 py-4 flex justify-between items-center transition-all">
        ...
        <div className="hidden md:flex items-center gap-8 text-xs font-black font-geist-mono uppercase tracking-[0.2em] text-zinc-500">
        ...
        <a 
          href={`https://wa.me/${config.contact.whatsapp}?text=Halo%20${encodeURIComponent(config.businessName)},%20saya%20ingin%20booking%20jadwal%20cukur`}
          target="_blank"
          rel="noreferrer"
      ...
      <section className="relative w-full min-h-[100dvh] flex flex-col md:flex-row border-b-2 border-zinc-800">
        ...
        <div className="w-full md:w-[55%] flex flex-col justify-end pt-32 pb-12 px-6 md:px-12 md:pb-24 border-r-2 border-zinc-800 z-10 bg-[#0a0a0a]">
          <motion.h1 
            ...
            className="text-[4rem] sm:text-[6rem] md:text-[7rem] lg:text-[10rem] font-black leading-[0.8] text-white tracking-tighter break-words uppercase font-geist-sans"
          >
          ...
          >
            <p className="text-lg md:text-xl text-zinc-400 font-medium leading-relaxed font-geist-mono uppercase tracking-tight">
              {config.tagline}
            </p>
          </motion.div>
        </div>
      ...
      <section id="about" className="flex flex-col md:flex-row border-b-2 border-zinc-800 bg-[#0a0a0a]">
        <div className="w-full md:w-1/3 p-6 md:p-12 border-b-2 md:border-b-0 md:border-r-2 border-zinc-800 flex items-start">
          <h2 className="text-xs font-black font-geist-mono uppercase tracking-[0.2em] text-zinc-500">
            [ About the Craft ]
          </h2>
        </div>
          ...
          {config.services.map((svc, i) => (
            ...
            >
              ...
              <div className="w-full sm:w-1/3 p-6 md:p-12 flex items-center justify-end sm:justify-start">
                <span className="text-2xl md:text-4xl font-black font-geist-mono text-zinc-600 group-hover:text-black transition-colors duration-500">
                  {svc.price}
                </span>
              </div>
            </motion.div>
            ...
            {config.catalog.map((item, i) => (
              <div key={i} className="group relative flex flex-col border-b-2 border-zinc-800 lg:border-b-0 lg:border-r-2 last:border-r-0 hover:bg-zinc-900 transition-colors duration-500">
                <div className="w-full aspect-square relative bg-zinc-950 overflow-hidden border-b-2 border-zinc-800">
                  <Image src={item.image} alt={item.name} fill className="object-cover grayscale contrast-125 group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]" />
                  <div className="absolute top-4 right-4 bg-white text-black px-3 py-1 text-xs font-black font-geist-mono uppercase tracking-widest">
                    {item.price}
                  </div>
                </div>
                <div className="p-8 flex flex-col justify-between flex-grow">
                  ...
                  <div className="mt-8 flex justify-end">
                    <a href={`https://wa.me/${config.contact.whatsapp}?text=I%20want%20to%20buy%20${item.name}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-white hover:text-zinc-400 uppercase text-xs font-black tracking-[0.2em] transition-colors">
          ...
          <div className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-6 px-6 md:px-12 pb-8">
            {config.images.gallery.map((img, i) => (
              <div key={i} className="flex-none w-[85vw] sm:w-[50vw] md:w-[35vw] snap-center relative aspect-[3/4] bg-zinc-900 overflow-hidden border-2 border-zinc-800 group">
                <Image src={img} alt="Gallery" fill className="object-cover grayscale contrast-[1.2] group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]" />
              </div>
            ))}
          </div>
        </section>
      ...
      <footer className="w-full bg-[#0a0a0a] flex flex-col md:flex-row relative z-10">
        <div className="w-full md:w-1/2 p-6 md:p-12 border-b-2 md:border-b-0 md:border-r-2 border-zinc-800 flex flex-col justify-between min-h-[400px]">
          ...
          <div className="flex flex-col gap-4 mt-12">
            <a href={config.contact.mapsLink} target="_blank" rel="noreferrer" className="flex items-center gap-4 text-sm font-black font-geist-mono uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-colors group w-fit">
              <MapPin size={20} className="group-hover:text-white transition-colors" /> Location
            </a>
            {config.contact.instagram && (
              <a href={config.contact.instagram} target="_blank" rel="noreferrer" className="flex items-center gap-4 text-sm font-black font-geist-mono uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-colors group w-fit">
                <InstagramLogo size={20} className="group-hover:text-white transition-colors" /> Instagram
              </a>
            )}
            {config.contact.facebook && (
              <a href={config.contact.facebook} target="_blank" rel="noreferrer" className="flex items-center gap-4 text-sm font-black font-geist-mono uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-colors group w-fit">
